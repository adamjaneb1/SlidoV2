  ...
  >
    ...
    >
      <template #item="{ element, index }">
        <div class="thumbnail-container bg-gray-50 hover:shadow-lg  my-1 rounded-lg mx-4">
          ...
          <div
            class="relative thumbnail-item rounded-lg  cursor-pointer"
            :class="{
          ...
          >
            ...
            <div class="w-7 pl-1 h-7  bg-black text-white rounded-tl  bg-opacity-60 drop-shadow-lg drop-shadow-white absolute top-1 left-1 z-10" :class="{ 'offset-left': index >= 99 }">{{ fillDigit(index + 1, 2) }}</div>

            <ThumbnailSlide class="thumbnail rounded-2xl " :slide="element" :size="215" :visible="index < slidesLoadLimit" />
...
watch(() => slideIndex.value, () => {
  ...
  // Check if current slide thumbnail is in viewport, scroll to position if not
  nextTick(() => {
...
const handleClickSlideThumbnail = (e: MouseEvent, index: number) => {
  ...
  // Hold Ctrl key to select slides, click again to deselect
  // If deselected slide is the active one, choose first selected slide as new active slide
  ...
  // Hold Shift key to select all slides in range
...
// Set thumbnail toolbar focus state (shortcuts only work when focused)
