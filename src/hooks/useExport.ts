...
export default () => {
  ...
  // Export slds file (unique .slds extension file)
  const exportSpecificFile = (_slides: Slide[]) => {
    const blob = new Blob([encrypt(JSON.stringify(_slides))], { type: '' })
    saveAs(blob, `${title.value}.slds`)
  }
  ...
  // Format color value to transparency + HexString for pptxgenjs use
  ...
  // Format HTML string to the format required by pptxgenjs
  // Core idea: Split HTML string into styled fragments, each fragment needs to inherit ancestor element styles, and wrap when encountering block-level elements
  ...
  // Check if it is a Base64 image URL
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          if (el.special) {
            ...
            if (svgRef.clientWidth < 1 || svgRef.clientHeight < 1) continue // Temporary handling (abnormal data from imported PPTX files)
